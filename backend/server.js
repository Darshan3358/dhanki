            ...
            if (user.referredBy) {
                ...
                const sponsorL1 = await User.findOne({
                    $or: [
                        ...
                        { referralId: cleanRefL1.toLowerCase() }
                    ]
                });
                if (sponsorL1) {
                    ...
                    if (sponsorL1.referredBy) {
                        ...
                        const sponsorL2 = await User.findOne({
                            $or: [
                                ...
                                { referralId: cleanRefL2.toLowerCase() }
                            ]
                        });
                        if (sponsorL2) {
                            ...
                            if (sponsorL2.referredBy) {
                                ...
                                const sponsorL3 = await User.findOne({
                                    $or: [
                                        ...
                                        { referralId: cleanRefL3.toLowerCase() }
                                    ]
                                });
                                if (sponsorL3) {
...
const runMigrations = async () => {
    try {
        ...
        // Update all existing users who have a walletAddress to use it as their referralId
        const usersWithWallet = await User.find({ walletAddress: { $ne: '' } });
        for (const user of usersWithWallet) {
            if (user.referralId !== user.walletAddress) {
                user.referralId = user.walletAddress.toLowerCase();
                await user.save();
            }
        }
        console.log('✅ Referral IDs updated to wallet addresses for existing users');
...
// Middleware
app.use(cors({
    origin: ['https://dhanik-bice.vercel.app', 'https://dhanik.vercel.app', 'http://localhost:5173', 'http://localhost:3000'],
    credentials: true
}));
